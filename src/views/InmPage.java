 ...
 * @author Nelkha
 */
public class InmPage extends javax.swing.JPanel {

    InmuebleServicio inmServ = new InmuebleServicio();
    ContratoServicio contServ = new ContratoServicio();
    DefaultTableModel dtm = new DefaultTableModel();

    public InmPage(String direccion) {
        initComponents();
        String[] titulos = new String[]{"ID", "INQ", "I.ACTUAL", "F. INCIO", "F.FIN"};
        ...
        Inmueble inmueble = inmServ.buscarInmuebleDireccion(direccion);
        if (inmueble != null) {
            List<Contrato> contratos = inmServ.buscarContratosInmuble(inmueble);
            lblInqTitle.setText(inmueble.getDireccion());
            for (Contrato cont : contratos) {
                dtm.addRow(new Object[]{cont.getId(), cont.getInquilino().getNombre()+" "+cont.getInquilino().getApellido(),
                    "$ " + formato.format(BigDecimal.valueOf(cont.getMontoAlquiler())), cont.getFechaInicio(), cont.getFechaFin()});
            }
        } else {
            lblInqTitle.setText("INMUEBLE INEXISTENTE");
        }
